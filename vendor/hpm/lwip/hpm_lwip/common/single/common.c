...
hpm_stat_t enet_init(ENET_Type *ptr)
{
    ...
    enet_tx_control_config.disable_crc = false;
    enet_tx_control_config.enable_crcr = false;
    ...
    /* Initialize phy */
        ...
        #elif defined(__USE_RTL8211) && __USE_RTL8211
        if (rtl8211_reset(ptr, RTL8211_ADDR) == false) {
            printf("Enet phy software reset failed !\n");
            return status_fail;
        }
        rtl8211_basic_mode_default_config(ptr, &phy_config);
        if (rtl8211_basic_mode_init(ptr, RTL8211_ADDR, &phy_config) == true) {
            /* Re-init RX DMA descriptors and restart RX DMA after PHY init.
               Flush cache so DMA can see the descriptor OWN bits and buffer addresses. */
            desc.rx_desc_list_cur = desc.rx_desc_list_head;
            for (uint32_t rxi = 0U; rxi < (uint32_t)ENET_RX_BUFF_COUNT; rxi++) {
                dma_rx_desc_tab[rxi].rdes0_bm.own = 1;
                dma_rx_desc_tab[rxi].rdes1_bm.rch = 1;
                dma_rx_desc_tab[rxi].rdes1_bm.rbs1 = ENET_RX_BUFF_SIZE;
                dma_rx_desc_tab[rxi].rdes2_bm.buffer1 = (uint32_t)&rx_buff[rxi][0];
                dma_rx_desc_tab[rxi].rdes3_bm.next_desc =
                    (rxi < (uint32_t)ENET_RX_BUFF_COUNT - 1U)
                        ? (uint32_t)&dma_rx_desc_tab[rxi + 1U]
                        : (uint32_t)&dma_rx_desc_tab[0U];
            }
            /* Flush descriptors to physical memory so DMA can see them */
            l1c_dc_writeback(HPM_L1C_CACHELINE_ALIGN_DOWN((uint32_t)dma_rx_desc_tab),
                             HPM_L1C_CACHELINE_ALIGN_UP(sizeof(dma_rx_desc_tab)));
            l1c_dc_writeback(HPM_L1C_CACHELINE_ALIGN_DOWN((uint32_t)rx_buff),
                             HPM_L1C_CACHELINE_ALIGN_UP(sizeof(rx_buff)));
            ENET->DMA_RX_DESC_LIST_ADDR = (uint32_t)desc.rx_desc_list_head;
            ENET->DMA_OP_MODE |= ENET_DMA_OP_MODE_SR_MASK;

            /* Flush TX descriptors and buffers so DMA can see buffer addresses
               and descriptor chain pointers written by enet_dma_tx_desc_chain_init.
               Without this, the DMA sees stale zeros and cannot transmit. */
            desc.tx_desc_list_cur = desc.tx_desc_list_head;
            l1c_dc_writeback(HPM_L1C_CACHELINE_ALIGN_DOWN((uint32_t)dma_tx_desc_tab),
                             HPM_L1C_CACHELINE_ALIGN_UP(sizeof(dma_tx_desc_tab)));
            l1c_dc_writeback(HPM_L1C_CACHELINE_ALIGN_DOWN((uint32_t)tx_buff),
                             HPM_L1C_CACHELINE_ALIGN_UP(sizeof(tx_buff)));
            ENET->DMA_TX_DESC_LIST_ADDR = (uint32_t)desc.tx_desc_list_head;
            ENET->DMA_OP_MODE |= ENET_DMA_OP_MODE_ST_MASK;
