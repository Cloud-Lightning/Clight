...
/*
 * SSC 5.12 compatibility:
 * Some SSC variants do not export EepromReadSize. Provide a weak fallback so
 * EEPROM emulation code can still build and run with default 16-bit access.
 */
#if defined(ESC_EEPROM_EMULATION) && ESC_EEPROM_EMULATION
ATTR_WEAK UINT16 EepromReadSize = EEPROM_WRITE_SIZE;
#endif

#define ECAT_LAN8720_REG_BMCR   (0U)
#define ECAT_LAN8720_REG_BMSR   (1U)
#define ECAT_LAN8720_REG_PHYID1 (2U)
#define ECAT_LAN8720_REG_PHYID2 (3U)
#define ECAT_LAN8720_REG_SMR    (18U)
#define ECAT_LAN8720_REG_PSCSR  (31U)

static const char *ecat_state_name(uint8_t state)
{
...
static const char *ecat_lan8720_speed_name(uint16_t pscsr)
{
    switch ((pscsr >> 2U) & 0x3U) {
    case 0x1U:
        return "10M";
    case 0x2U:
        return "100M";
...
static void ecat_debug_dump_lan8720_mdio(ESC_Type *esc_ptr, const char *tag)
{
    uint16_t bmcr = 0;
    uint16_t bmsr = 0;
    uint16_t phyid1 = 0;
    ...
    stat = esc_mdio_read(esc_ptr, BOARD_ECAT_PORT0_PHY_ADDR, ECAT_LAN8720_REG_BMCR, &bmcr);
    stat |= esc_mdio_read(esc_ptr, BOARD_ECAT_PORT0_PHY_ADDR, ECAT_LAN8720_REG_BMSR, &bmsr);
    stat |= esc_mdio_read(esc_ptr, BOARD_ECAT_PORT0_PHY_ADDR, ECAT_LAN8720_REG_BMSR, &bmsr); /* BMSR link bit is latched low */
    stat |= esc_mdio_read(esc_ptr, BOARD_ECAT_PORT0_PHY_ADDR, ECAT_LAN8720_REG_PHYID1, &phyid1);
    stat |= esc_mdio_read(esc_ptr, BOARD_ECAT_PORT0_PHY_ADDR, ECAT_LAN8720_REG_PHYID2, &phyid2);
    stat |= esc_mdio_read(esc_ptr, BOARD_ECAT_PORT0_PHY_ADDR, ECAT_LAN8720_REG_SMR, &smr);
    stat |= esc_mdio_read(esc_ptr, BOARD_ECAT_PORT0_PHY_ADDR, ECAT_LAN8720_REG_PSCSR, &pscsr);
    if (stat != status_success) {
        printf("[ECAT/%s] LAN8720 MDIO read failed\n", tag);
        return;
    }

    printf("[ECAT/%s] LAN8720 BMCR=0x%04x BMSR=0x%04x PHYID=%04x:%04x SMR=0x%04x PSCSR=0x%04x an_en=%u an_done=%u link=%u duplex=%s speed=%s\n",
           tag,
           bmcr,
           bmsr,
           phyid1,
           ...
           smr,
           pscsr,
           (unsigned) ((bmcr >> 12U) & 0x1U),
           (unsigned) ((pscsr >> 12U) & 0x1U),
           (unsigned) ((bmsr >> 2U) & 0x1U),
           (((pscsr >> 4U) & 0x1U) != 0U) ? "full" : "half",
           ecat_lan8720_speed_name(pscsr));
}

static void ecat_debug_dump_esc_status(ESC_Type *esc_ptr, const char *tag)
{
    const uint16_t dl = esc_ptr->ESC_DL_STAT;
    const uint16_t al = esc_ptr->AL_STAT;
    const uint16_t al_code = esc_ptr->AL_STAT_CODE;
    const uint8_t phy0 = esc_ptr->PHY_STAT[ESC_PHY_STAT_PORT0];
    const uint8_t lost0 = esc_ptr->LOST_LINK_CNT[ESC_LOST_LINK_CNT_PORT0];

    printf("[ECAT/%s] AL=0x%04x(%s err=%u devid=%u) ALCODE=0x%04x DL=0x%04x P0[plp=%u cp=%u lp=%u phy_pls=%u phy_ls=%u lse=%u re=%u lost=%u] PDI=0x%02x PHY_CFG0=0x%08lx PHY_CFG1=0x%08lx GPR_CFG2=0x%08lx GPR_STATUS=0x%08lx\n",
           tag,
           al,
           ecat_state_name((uint8_t) ESC_AL_STAT_AS_GET(al)),
           (unsigned) ESC_AL_STAT_EI_GET(al),
           (unsigned) ESC_AL_STAT_DI_GET(al),
           al_code,
           dl,
           (unsigned) ESC_ESC_DL_STAT_PLP0_GET(dl),
           (unsigned) ESC_ESC_DL_STAT_CP0_GET(dl),
           (unsigned) ESC_ESC_DL_STAT_LP0_GET(dl),
           (unsigned) ESC_PHY_STAT_PLS_GET(phy0),
           (unsigned) ESC_PHY_STAT_LS_GET(phy0),
           (unsigned) ESC_PHY_STAT_LSE_GET(phy0),
           (unsigned) ESC_PHY_STAT_RE_GET(phy0),
           (unsigned) lost0,
           (unsigned) ESC_PDI_CTRL_PDI_GET(esc_ptr->PDI_CTRL),
           (unsigned long) esc_ptr->PHY_CFG0,
           (unsigned long) esc_ptr->PHY_CFG1,
           (unsigned long) esc_ptr->GPR_CFG2,
           (unsigned long) esc_ptr->GPR_STATUS);
}
...
void ecat_debug_dump_startup(ESC_Type *esc_ptr)
{
    ...
    ecat_debug_dump_esc_status(esc_ptr, "BOOT");
    ecat_debug_dump_lan8720_mdio(esc_ptr, "BOOT");
}

void ecat_debug_poll(ESC_Type *esc_ptr)
{
    static bool first = true;
    static uint16_t last_dl = 0xffffU;
    static uint16_t last_al = 0xffffU;
    static uint16_t last_al_code = 0xffffU;
    static uint8_t last_phy0 = 0xffU;
    ...
    const uint16_t al_code = esc_ptr->AL_STAT_CODE;
    const uint8_t phy0 = esc_ptr->PHY_STAT[ESC_PHY_STAT_PORT0];
    const bool changed = first || (dl != last_dl) || (al != last_al) || (al_code != last_al_code) || (phy0 != last_phy0);
    const bool heartbeat = (!first) && ((now_ms - last_heartbeat_ms) >= 1000U);

    if (!changed && !heartbeat) {
        return;
    }

    ecat_debug_dump_esc_status(esc_ptr, changed ? "STAT" : "HB");
    if (changed || ((now_ms - last_heartbeat_ms) >= 3000U)) {
        ecat_debug_dump_lan8720_mdio(esc_ptr, changed ? "STAT" : "HB");
    }

    first = false;
    last_dl = dl;
    last_al = al;
    last_al_code = al_code;
    last_phy0 = phy0;
...
void ecat_preconfigure_process_data_sm(ESC_Type *esc_ptr, UINT16 output_size, UINT16 input_size)
{
    ...
    sm2 = GetSyncMan(PROCESS_DATA_OUT);
    sm3 = GetSyncMan(PROCESS_DATA_IN);
    if ((sm2 == NULL) || (sm3 == NULL)) {
        return;
    }

    if (output_size > 0U) {
        sm2->AddressLength = (((UINT32) output_size) << SM_LENGTH_SHIFT) | 0x1100U;
        sm2->Settings[0] = 0x00010064U;
    }

    if (input_size > 0U) {
        sm3->AddressLength = (((UINT32) input_size) << SM_LENGTH_SHIFT) | 0x1400U;
        sm3->Settings[0] = 0x00010020U;
    }

    printf("[ECAT/SMCFG] preload SM2 addr=0x1100 len=%u ctrl=0x0064 en=1, SM3 addr=0x1400 len=%u ctrl=0x0020 en=1\n",
           output_size,
           input_size);
}
...
hpm_stat_t ecat_hardware_init(ESC_Type *esc_ptr)
{
    ...
    esc_config.phy_refclk_en = false;
    ...
    /* Custom board uses LAN8720 on ESC port0 in RMII mode. Force-enable RMII. */
    esc_ptr->PHY_CFG0 |= ESC_PHY_CFG0_PORT0_RMII_EN_MASK;
    /* RMII 50MHz reference clock is now wired to P0_TXCK (PA24). */
    esc_ptr->PHY_CFG1 &= ~(ESC_PHY_CFG1_REFCK_25M_OE_MASK
                        | ESC_PHY_CFG1_REFCK_25M_INV_MASK
                        | ESC_PHY_CFG1_RMII_REFCLK_SEL_MASK
                        | ESC_PHY_CFG1_RMII_P0_TXCK_REFCLK_OE_MASK
                        | ESC_PHY_CFG1_RMII_P0_RXCK_REFCLK_OE_MASK);
    esc_ptr->PHY_CFG1 |= ESC_PHY_CFG1_RMII_REFCLK_SEL_SET(1U);
...
void ecat_get_led_status(ecat_led_status_t *status)
{
    ...
    status->run_led_raw = (uint8_t) (u8EcatRunLed & (uint8_t) (~LED_OVERRIDE));
    status->error_led_raw = (uint8_t) (u8EcatErrorLed & (uint8_t) (~LED_OVERRIDE));
    status->run_active = (status->run_led_raw != LED_OFF);
    status->error_active = (status->error_led_raw != LED_OFF);
}
