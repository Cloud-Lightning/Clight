...
void ResetALEventMask(UINT16 intMask)
{
    ...
    u32Mask &= ~(SYNC0_EVENT | SYNC1_EVENT); /* mask Sync0/Sync1 for PDI IRQ; dedicated Sync0/Sync1 IRQs are used */
...
UINT8    CheckSmSettings(UINT8 maxChannel)
{
    ...
    if ((nAlStatus == STATE_PREOP) && (SMLength == 0U) && (nPdInputSize > 0U)) {
        pSyncMan->AddressLength = (((UINT32)nPdInputSize) << SM_LENGTH_SHIFT) | 0x1400U;
        pSyncMan->Settings[0] = 0x00010020U;
        SMLength = nPdInputSize;
        SMAddress = 0x1400U;
        printf("[ECAT/SM3] autofix addr=0x1400 len=%u ctrl=0x0020 en=1\n", nPdInputSize);
    }
    ...
    if (
#if defined(ECAT_TEMP_MAILBOX_ONLY) && ECAT_TEMP_MAILBOX_ONLY
        !(nPdInputSize == 0 && SMLength == 0) &&
#endif
        (pSyncMan->Settings[SM_SETTING_ACTIVATE_OFFSET] & SM_SETTING_ENABLE_VALUE) != 0 && SMLength == 0)
    {
        ...
        result = SYNCMANCHSETTINGS + 1;
            ...
            !(nPdInputSize == 0 && SMLength == 0) &&
#endif
            (pSyncMan->Settings[SM_SETTING_ACTIVATE_OFFSET] & SM_SETTING_ENABLE_VALUE))
        {
        ...
        if ( result != 0 )
        {
            printf("[ECAT/SM3] raw=%u SMLength=%u nPdInputSize=%u SMAddress=0x%04x expectedInputAddr=0x%04x outAddr=0x%04x ctrl=0x%02x act=0x%02x state=0x%02x\n",
                   result,
                   SMLength,
                   nPdInputSize,
                   SMAddress,
                   nEscAddrInputData,
                   nEscAddrOutputData,
                   pSyncMan->Settings[SM_SETTING_CONTROL_OFFSET],
...
//    else
    ...
    if ((nAlStatus == STATE_PREOP) && (SMLength == 0U) && (nPdOutputSize > 0U)) {
        pSyncMan->AddressLength = (((UINT32)nPdOutputSize) << SM_LENGTH_SHIFT) | 0x1100U;
        pSyncMan->Settings[0] = 0x00010064U;
        SMLength = nPdOutputSize;
        SMAddress = 0x1100U;
        printf("[ECAT/SM2] autofix addr=0x1100 len=%u ctrl=0x0064 en=1\n", nPdOutputSize);
    }


    if (
#if defined(ECAT_TEMP_MAILBOX_ONLY) && ECAT_TEMP_MAILBOX_ONLY
        !(nPdOutputSize == 0 && SMLength == 0) &&
#endif
        (pSyncMan->Settings[SM_SETTING_ACTIVATE_OFFSET] & SM_SETTING_ENABLE_VALUE) != 0 && SMLength == 0)
    {
...
UINT16 StartInputHandler(void)
{
    ...
    */
    ...
    {
        printf("[ECAT/IN] overlap inputAddr=0x%04x inputSize=%u inputBuf=%u sendMbx=0x%04x+%u recvMbx=0x%04x+%u\n",
               nEscAddrInputData,
               nPdInputSize,
               nPdInputBuffer,
               u16EscAddrSendMbx,
